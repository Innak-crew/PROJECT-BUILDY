...
<div class="px-4 py-3 border-bottom d-flex justify-content-between align-items-center">
  ...
  <a href="{{route('admin.new.product')}}" class="btn btn-success font-medium rounded-pill px-4">Add new</a>
    ...
    <div class="card-body p-4">
        ...
        <table class="table border text-nowrap customize-table mb-0 align-middle @if ($pageData->Products && count($pageData->Products) != 0) datatable-select-inputs @endif">
  <thead class="text-dark fs-4">
    <tr>
      ...
      <th>
        <h6 class="fs-4 fw-semibold mb-0">Rate per</h6>
      </th>
      <th>
  ...
  <tbody>
    @if ($pageData->Products && count($pageData->Products) != 0)
        ...
        <tr>
          ...
          <td>
            <p class="mb-0 fw-normal fs-4"><img src="{{ $pageData->Products[$i]->image_url }}" width="50px"></p>
          </td>
          <td>
            <p class="mb-0 fw-normal fs-4">{{ $pageData->Products[$i]->name }}</p>
          </td>
          <td>
            <p class="mb-0 fw-normal fs-4">{{ $pageData->Products[$i]->type }}</p>
          </td>
          <td>
            <p class="mb-0 fw-normal fs-4">{{ $pageData->Products[$i]->unit()->name }}</p>
          </td>
          <td>
            <p class="mb-0 fw-normal fs-4">{{ $pageData->Products[$i]->rate_per }}</p>
          </td>
          <td class="">
            <a href="{{route('admin.view.product',['encodedId' => base64_encode($pageData->Products[$i]->id)])}}" class="text-success"><svg  xmlns="http://www.w3.org/2000/svg"  width="24"  height="24"  viewBox="0 0 24 24"  fill="none"  stroke="currentColor"  stroke-width="2"  stroke-linecap="round"  stroke-linejoin="round"  class="icon icon-tabler icons-tabler-outline icon-tabler-eye"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M10 12a2 2 0 1 0 4 0a2 2 0 0 0 -4 0" /><path d="M21 12c-2.4 4 -5.4 6 -9 6c-3.6 0 -6.6 -2 -9 -6c2.4 -4 5.4 -6 9 -6c3.6 0 6.6 2 9 6" /></svg></a>
            <a href="{{route('admin.edit.product',['encodedId' => base64_encode($pageData->Products[$i]->id)])}}" class="text-primary mx-2"><svg  xmlns="http://www.w3.org/2000/svg"  width="24"  height="24"  viewBox="0 0 24 24"  fill="none"  stroke="currentColor"  stroke-width="2"  stroke-linecap="round"  stroke-linejoin="round"  class="icon icon-tabler icons-tabler-outline icon-tabler-edit"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M7 7h-1a2 2 0 0 0 -2 2v9a2 2 0 0 0 2 2h9a2 2 0 0 0 2 -2v-1" /><path d="M20.385 6.585a2.1 2.1 0 0 0 -2.97 -2.97l-8.415 8.385v3h3l8.385 -8.415z" /><path d="M16 5l3 3" /></svg></a>
            <form action="{{route('product.destroy', ['encodedId' => base64_encode($pageData->Products[$i]->id)])}} "
